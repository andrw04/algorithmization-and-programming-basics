...
void MainWindow::Normalize(std::vector<int>& c) {
...
void MainWindow::AddZero(std::vector<int>& c, int x) {
    c.resize(x + 1);
    c[x] = 0;
}

std::vector<int> MainWindow::AddNum(const std::vector<int>& a, const std::vector<int>& b, int k) {
...
int MainWindow::GetBit(const std::vector<int>& v, int b) {
    return v[b] & 1;
}

std::vector<int> MainWindow::Multiply(const std::vector<int>& a, int x, int k) {
...
void MainWindow::GenerateSequence(std::vector<int> n, const std::vector<std::vector<int>>& p, int k, int i, std::vector<std::pair<int, std::vector<int>> >& v) {
...
void MainWindow::on_pushButton_clicked()
{
    size_t num = QInputDialog::getText(this,"Enter the number :","Num(from 1 to 10000):",QLineEdit::Normal).toShort();
         ...
         QString finalAnswer;
         --num;
         std::vector<std::pair<int, std::vector<int>>> sequence;

         for (int k = 1; num / 2 >= sequence.size(); ++k) {
            std::vector<int> a(k + 1);
             a[k] = 1;
             std::vector<std::vector<int>> b(k + 1);
             b[0].push_back(1);
             AddZero(b[0], k + 1);
             for (int i = 0; i < k; i++) {
         ...
         --num;
         std::vector<int> answer = sequence[num / 2].second;
